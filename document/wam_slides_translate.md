...
## <a name="chapter2">2.Languege L2: Flat Resolution </a>
  ...
  - L2は、L1をさらに拡張し、手続き処理として bodyを持っている場合まで拡張する。
...
```
    call p/n ≡ CP ← P + instruction_size(P);  (訳註: 例えば?-a(X),b(X). の場合 Pはa/1のアドレス、CPはb/1のアドレスがセットされる）
                P ← @(p/n);
...
### 【課題(Problem)】
同じ変数レジスタが、各body と goalで使用されるため、permanent 変数は、上書されるリスクがある。

        例: p(X,Y) :- q(X,Z),r(Z,Y).

  変数Y,Zのレジスタ値が、q実行の後もなお
...
M2は、手続きの permanent 変数とレジスタCPを、ランタイム・スタックに保存する。  

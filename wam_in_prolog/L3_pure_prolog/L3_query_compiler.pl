:- module(l3_query_compiler, [compile_query/2]).
% ?- query_compile(p(Z,h(Z,W),f(W)),L),write(L).

:- dynamic cnt/1.

compile_query((A,B),L3):-
  compile_query(A,L),
...
compile_query((RootPropaty),L):-
  ...
  var_assign(S_OUT_D2,S_OUT_D3,VL,_),
...
reg_assign([put_structure(N/A)=Arg],[(put_structure(N/A),Arg)],V,V).
